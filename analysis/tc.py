...
def cpu_time(folder, dataset="Reddit"):
    outfile_path = osp.join(
        osp.dirname(osp.realpath(__file__)), "tc", dataset, "cpu_time.txt"
    )
    ...
    with open(osp.join(folder, dataset, "baseline/stdout.txt")) as f:
        ...
        relabel_time = [
            x
            for x in relabel_time
            if x < relabel_time_mean + 1.5 * relabel_time_std
            and x > relabel_time_mean - 1.5 * relabel_time_std
        ]
        outfile.write(
            f"baseline: {np.mean(cpu_time) :.3f} ({num_trail - len(cpu_time)} Outliers), {np.mean(relabel_time) :.3f} ({num_trail - len(cpu_time)} Outliers), {np.mean(cpu_time) + np.mean(relabel_time) :.3f}\n\n"
        )
        x.append(0)
    ...
    for prune_algo in ["random", "in_degree", "out_degree", "er"]:
        ...
        for subdir in sorted(os.listdir(osp.join(folder, dataset, prune_algo))):
            ...
            with open(osp.join(folder, dataset, prune_algo, subdir, "stdout.txt")) as f:
                ...
                relabel_time = (
                    [0] * num_trail if len(relabel_time) == 0 else relabel_time
                )
                outfile.write(
                    f"{subdir}: {np.mean(cpu_time) :.3f} ({num_trail - len(cpu_time)} Outliers), {np.mean(relabel_time) :.3f} ({num_trail - len(relabel_time)} Outliers), {np.mean(cpu_time) + np.mean(relabel_time) :.3f}\n"
                )
...
def triangle_count(folder, dataset="Reddit"):
    outfile_path = osp.join(
        osp.dirname(osp.realpath(__file__)), "tc", dataset, "triangle_count.txt"
    )
    os.makedirs(osp.dirname(outfile_path), exist_ok=True)
    outfile = open(outfile_path, "w")
