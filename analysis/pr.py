...
def cpu_time(folder, dataset="Reddit"):
    outfile_path = osp.join(osp.dirname(osp.realpath(__file__)), "pr", dataset, 'cpu_time.txt')
    ...
    with open(osp.join(folder, dataset, 'baseline/stdout.txt')) as f:
        ...
        outfile.write(f"baseline: {np.mean(cpu_time) :.2f} ({num_trail - len(cpu_time)} Outliers), {total_iteration / num_trail}, {np.mean(cpu_time)/(total_iteration/num_trail):.4f}\n\n")
        x.append(0)
    ...
    for prune_algo in ["random", "in_degree", "out_degree", "old_er", "er"]:
        ...
        for subdir in sorted(os.listdir(osp.join(folder, dataset, prune_algo))):
            ...
            with open(osp.join(folder, dataset, prune_algo, subdir, 'stdout.txt')) as f:
                ...
                cpu_time_std = np.std(cpu_time)
                cpu_time = [x for x in cpu_time if x < cpu_time_mean + 1.5 * cpu_time_std and x > cpu_time_mean - 1.5 * cpu_time_std]
                outfile.write(f"{subdir}: {np.mean(cpu_time) :.2f} ({num_trail - len(cpu_time)} Outliers), {total_iteration / num_trail}, {np.mean(cpu_time)/(total_iteration/num_trail):.4f}\n")
...
def relative_score_error(folder, dataset='Reddit', prune_algo='random'):
    ...
    save_path = osp.join(osp.dirname(osp.realpath(__file__)), "pr", dataset, prune_algo, 'relative_score_error.png')
...
def precision_k(folder, percent, dataset='Reddit', prune_algo='random'):    
    ...
    save_path = osp.join(osp.dirname(osp.realpath(__file__)), "pr", dataset, prune_algo, 'precision_k.png')
    os.makedirs(osp.dirname(save_path), exist_ok=True)
