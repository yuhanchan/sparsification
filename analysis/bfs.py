...
def cpu_time(folder, dataset="Reddit"):
    outfile_path = osp.join(osp.dirname(osp.realpath(__file__)), "bfs", dataset, 'cpu_time.txt')
...
def reachability(folder, dataset="Reddit"):
    outfile_path = osp.join(osp.dirname(osp.realpath(__file__)), "bfs", dataset, 'reachability.txt')
    os.makedirs(osp.dirname(outfile_path), exist_ok=True)
    outfile = open(outfile_path, 'w')
    outfile.write('prune rate: #Reachable_Nodes/#Nodes (percent), #Reachable_Edges/#Edges (percent)\n\n')
    ...
    outfile.write(f"baseline: {reachable_num_nodes}/{total_num_nodes} ({reachable_num_nodes/total_num_nodes*100:.2f}%), {reachable_num_edges}/{total_num_edges} ({reachable_num_edges/total_num_edges*100:.2f}%)\n\n")
    x.append(0)
    ...
    for prune_algo in ["random", "in_degree", "out_degree", "er"]:
        ...
        for subdir in sorted(os.listdir(osp.join(folder, dataset, prune_algo))):
            ...
            with open(osp.join(folder, dataset, prune_algo, subdir, 'analysis.txt')) as f:
                ...
                reachable_num_nodes = int(line[3])
                reachable_num_edges = int(line[6])
                outfile.write(f"{subdir}: {reachable_num_nodes}/{total_num_nodes} ({reachable_num_nodes/total_num_nodes*100:.2f}%), {reachable_num_edges}/{total_num_edges} ({reachable_num_edges/total_num_edges*100:.2f}%)\n")
                x.append(float(subdir))
                y.append(reachable_num_nodes/total_num_nodes*100)
        plot_dict[prune_algo] = [plot_dict['baseline'][0] + x, plot_dict['baseline'][1] + y]
        outfile.write("\n")
