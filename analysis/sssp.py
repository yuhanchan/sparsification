...
def relative_score_error(folder, dataset='Reddit', prune_algo='random'):
    ...
    save_path = osp.join(osp.dirname(osp.realpath(__file__)), "sssp", dataset, prune_algo, 'relative_score_error.png')
    ...
    with ProcessPoolExecutor(max_workers=1) as executor:
        ...
        for ds in ['Reddit', 'Reddit2', 'ogbn_products']:
            for pa in ['random', 'in_degree', 'out_degree', 'old_er', 'er']:
                ...
                futures[executor.submit(relative_score_error, '../experiments/sssp', ds, pa)] = f"dataset={ds}, prune_algo={pa}, metric=relative_score_error"
        # run jobs
