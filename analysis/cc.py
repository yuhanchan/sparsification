...
def cpu_time(folder, dataset="Reddit"):
    outfile_path = osp.join(osp.dirname(osp.realpath(__file__)), "cc", dataset, 'cpu_time.txt')
...
def component_count(folder, dataset="Reddit"):
    outfile_path = osp.join(osp.dirname(osp.realpath(__file__)), "cc", dataset, 'component_count.txt')
    os.makedirs(osp.dirname(outfile_path), exist_ok=True)
    outfile = open(outfile_path, 'w')
