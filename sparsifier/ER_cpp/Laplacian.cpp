...
int keyMap(int x, int n) { return x <= n ? x : n + x / n; }
...
double norm(vector<double> &a) { return sqrt(dot(a, a)); }
...
ApproxCholPQ_t ApproxCholPQ(vector<int> &degs) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (head >= 0) {
      ...
      res.elems[head] = new ApproxCholPQElem_t(i, res.elems[head]->next,
                                               res.elems[head]->key);
    } else {
...
vector<double> LDLSolver(LDLinv_t &ldli, vector<double> &b) {
  ...
  double mu =
      accumulate(y.begin(), y.end(), 0.0) / static_cast<double>(y.size());
...
vector<double> pcg(SparseMatrixCSC &mat, vector<double> b, LDLinv_t &ldli,
                   bool verbose = false, double tol = 0.01, int maxits = 1000,
                   int stag_test = 5) {
  double al = 0;
...
#pragma omp parallel for
  ...
  for (int i = 0; i < bs.size(); i++) {
    ...
    double b_mean =
        accumulate(b.begin(), b.end(), 0.0) / static_cast<double>(b.size());
...
#pragma omp critical
  cout << "PCG total time: "
       << chrono::duration_cast<chrono::milliseconds>(e1 - s1).count() << " ms"
       << endl;
#endif
