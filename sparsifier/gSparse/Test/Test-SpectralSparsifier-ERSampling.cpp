...
TEST(ERSampling, Getters) {
  ...
  // Call constructors
  gSparse::Graph test(new gSparse::UndirectedGraph(Edges, Weights));
  gSparse::SpectralSparsifier::ERSampling sparsifier(
      test, 5.0, 0.5, gSparse::SpectralSparsifier::EXACT_ER);
...
TEST(ERSampling, Setters) {
  ...
  EXPECT_EQ(gSparse::SpectralSparsifier::APPROXIMATE_ER,
            sparsifier.GetERPolicy());
  sparsifier.SetC(5.0);
