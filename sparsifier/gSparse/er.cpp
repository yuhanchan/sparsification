...
int main(int argc, char *argv[]) {
  if (argc != 6) {
    std::cout << "Usage: " << argv[0]
              << " <epsilon> <input el path> <output el path> <output weight "
                 "path> <er save path>"
              << std::endl;
    return -1;
  }
  // Create a CSV Reader and specify the file location
  gSparse::GraphReader csvReader =
      std::make_shared<gSparse::GraphCSVReader>(argv[2], "None", " ");
  ...
  float prune_rate =
      1.0 - float(sparseGraph1->GetEdgeCount()) / float(graph->GetEdgeCount());
  std::cout << "Epsilon = " << argv[1] << ", Sparised Edge Count (ApproxER): "
            << sparseGraph1->GetEdgeCount() << " prunerate = " << prune_rate
            << std::endl;
  std::cout
      << "GetSparsifiedGraph time: "
      << std::chrono::duration_cast<std::chrono::seconds>(end - start).count()
      << " seconds" << std::endl;

  gSparse::GraphWriter csvWriter =
      std::make_shared<gSparse::GraphCSVWriter>(argv[3], argv[4], " ");

  start = std::chrono::high_resolution_clock::now();
  // csvWriter->Write(sparseGraph1->GetEdgeList(),
  // sparseGraph1->GetWeightList());
  end = std::chrono::high_resolution_clock::now();
  std::cout
      << "Write time: "
      << std::chrono::duration_cast<std::chrono::seconds>(end - start).count()
