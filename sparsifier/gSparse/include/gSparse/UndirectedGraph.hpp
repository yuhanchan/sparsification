...
/*!
    This class provides a multiple representation of an Undirected, Simple
   graph.
*/
...
public:
  ...
  //! Graph data type should not be copied by value / reassigne due to Eigen
  //! data type
  ...
  /*!
  \param DataReader: A subclass of IGraphReader which provides an interface to
  read external data.
  */
  UndirectedGraph(const gSparse::GraphReader &DataReader) {
    if (DataReader == nullptr) {
      throw std::invalid_argument(
          "UndirectedGraph: DataReader must not be NULL");
    }
  ...
  //! A constructor to initialize graph based from Edge data. Weight sets to
  //! one.
  /*!
  ...
  */
  UndirectedGraph(const gSparse::EdgeMatrix &Edges,
      ...
      : _edges(Edges), _weights(Weights) {
    _initializeSystem();
  }
  //! Return Graph's Adjancency Matrix
  virtual inline const gSparse::SparsePrecisionMatrix &
  GetAdjacentMatrix() const {
    return _adjMatrix;
  }
  //! Return Graph's Incident Matrix
  virtual inline const gSparse::SparsePrecisionMatrix &
  GetIncidentMatrix() const {
    return _incidentMatrix;
  }
  //! Return Graph's Degree Matrix
  virtual inline const gSparse::SparsePrecisionMatrix &GetDegreeMatrix() const {
    return _degMatrix;
  }
  //! Return Graph's Weight Matrix
  virtual inline const gSparse::SparsePrecisionMatrix &GetWeightMatrix() const {
    return _weightMatrix;
  }
  //! Return Graph's Laplacian Matrix
  virtual inline const gSparse::SparsePrecisionMatrix &
  GetLaplacianMatrix() const {
    return _laplacianMatrix;
  }
  //! Return Graph's Edge List Matrix
  virtual inline const gSparse::EdgeMatrix &GetEdgeList() const {
    return _edges;
  }
  //! Return Graph's Weight List Matrix
  virtual inline const gSparse::PrecisionRowMatrix &GetWeightList() const {
    return _weights;
  }
...
protected:
  gSparse::SparsePrecisionMatrix
      _adjMatrix; //!< adjacency matrix representation
  gSparse::SparsePrecisionMatrix _degMatrix; //!< degree matrix representation
  gSparse::SparsePrecisionMatrix
      _incidentMatrix; //!< incident matrix representation
  gSparse::SparsePrecisionMatrix
      _weightMatrix; //!< weight matrix representation
  gSparse::SparsePrecisionMatrix
      _laplacianMatrix; //!< laplacian matrix representation
...
private:
  ...
  virtual inline void _initializeSystem() {
    ...
    // Some of the checks are expensive such as checking minCoeff with O(n)
    // runtime.
    _validateInput();
#endif
  ...
  void inline _validateInput() {
    ...
    if (_edges.rows() != _weights.rows()) {
      std::stringstream ss;
      ss << "UndirectedGraph: Edges.rows(): " << _edges.rows()
         << " =/= Weights.rows() " << _weights.rows() << std::endl;
  ...
  void inline _initializeMatrixSystem() {
    // Calculate counts
    _nodeCount =
        static_cast<std::size_t>(std::max(_edges.leftCols(1).maxCoeff(),
                                          _edges.rightCols(1).maxCoeff()) +
                                 1);
    ...
    for (std::size_t i = 0; i != _edgeCount; ++i) {
      ...
      adjacentList.push_back(
          Eigen::Triplet<gSparse::PRECISION>(r, c, _weights(i, 0)));
      adjacentList.push_back(
          Eigen::Triplet<gSparse::PRECISION>(c, r, _weights(i, 0)));
      ...
      weightList.push_back(
          Eigen::Triplet<gSparse::PRECISION>(i, i, _weights(i)));
    }
    // Create adj matrix
    ...
    for (int i = 0; i != degVector.size(); ++i) {
      degreeList.push_back(
          Eigen::Triplet<gSparse::PRECISION>(i, i, degVector(i)));
    }
