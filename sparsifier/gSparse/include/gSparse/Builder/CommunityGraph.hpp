...
///
void createRandomGraph(int total_community = 3, int bridges = 4,
                       int community_size = 30,
                       const std::string &name = "random.csv") {
  ...
  for (int i = 1; i != 2; ++i) {
    ...
    // Each community is currently a complete graph to represent a dense graph.
    // This can be sparsify safety through uniform sampling.
    ...
    for (int community_id = 0; community_id != total_community;
         ++community_id) {
      std::stringstream ss;
      ...
      for (int k = 0; k != total_community; ++k) {
        if (k > j) {
          for (int l = 0; l != bridges; ++l) {
            std::stringstream ss;
            ss << j * offset + distribution(generator) << " "
               << k * offset + distribution(generator);
            myfile << stringCleaner(ss.str()) << std::endl;
          }
        }
      }
