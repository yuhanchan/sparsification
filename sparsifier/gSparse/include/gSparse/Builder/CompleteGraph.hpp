...
/// This function builds a unit complete graph
/// \param nodeCount  Number of nodes in the Complete graph
/// \param lower_weight lower bound of the weight to randomize. The value must
/// be greater than zero. \param upper_weight upper bound of the weight to
/// randomize. The value must be greater than zero.
inline gSparse::Graph buildRandomCompleteGraph(std::size_t nodeCount,
                                               double lower_weight,
                                               double upper_weight) {
  std::size_t &n = nodeCount;
  ...
  gSparse::EdgeMatrix resultEdge = gSparse::EdgeMatrix((n * (n - 1)) / 2, 2);
  gSparse::PrecisionRowMatrix resultWeight =
      gSparse::PrecisionRowMatrix((n * (n - 1)) / 2, 1);
  std::size_t row = 0;
