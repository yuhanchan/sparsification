...
/*!
    ...
    Node index starts from zero. Weight data type is gSparse::PRECISION type
...
public:
  ...
  */
  GraphCSVWriter(const std::string &EdgeFileName,
      ...
      : _delim(delimeter), _edgeFile(EdgeFileName),
  ...
  */
  virtual void inline Write(const gSparse::Graph &graph) {
    ...
    // Illegal use of this function. Graph object should have weights and must
    // be written.
    if (_weightFile == "None") {
      throw std::invalid_argument("GraphCSVWriter: Unable to write weight "
                                  "without destination filename");
    }
    // Write the Edge List
