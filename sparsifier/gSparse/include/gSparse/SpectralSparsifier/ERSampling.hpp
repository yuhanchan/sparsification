...
///
/// There are many different approaches to calculate Effective Resistance of a
/// graph. This is enum type defines the method used by ERSampling class
///
enum ER_METHODS { APPROXIMATE_ER = 0, EXACT_ER = 1 };
...
protected:
  ...
  gSparse::EffectiveResistance
      _erCalculator; //!< Pointer to EffectiveResistance module

  gSparse::SpectralSparsifier::ER_METHODS
      _erPolicy; //!< EffectiveResistance Calculation Policy
public:
  ///
  /// Constructor to create a ISparsifier object to perform Spectral
  /// Sparsification by Effective Resistance
  ///
  /// \param graph    Smart Pointer to the IGraph object. This is the dense
  /// graph for the sparsifier. \param C        C hyper-parameter of Spectral
  /// Sparsifier by Effective Resistance.
  ///                 As described by paper. This should be a large constant.
  ///                 Default value is 4.0.
  /// \param Epsilon  Epsilon hyper-parameter. This should be between 0.0
  /// and 1.0
  ///                 The sparsifier will generate (m log m ) / (Epsilon^2)
  ///                 edges. Thus, lower epsilon will yeild denser sparsifier.
  ///                 High epsilon could yeild a disconnect graph.
  /// \param ERPolicy EffectiveResistance calculation method.
  ///                 APPROXIMATE_ER:  Default. Approximate EffectiveResistance
  ///                 with an error. This should be sufficient for Spectral
  ///                 Sparsifier. EXACT_ER:  Calculate graph exact
  ///                 EffectiveResistance. This is very slow and not
  ///                 recommended.
  ERSampling(const gSparse::Graph &graph, double C = 4.0f,
             double Epsilon = 0.3f,
             gSparse::SpectralSparsifier::ER_METHODS ERPolicy =
                 gSparse::SpectralSparsifier::APPROXIMATE_ER) {
    SetC(C);
    SetEpsilon(Epsilon);
    _graph = graph;
  ...
  virtual inline void SaveER(std::ofstream &ofs) {
    ...
    // {
    //     ofs << _graph->GetEdgeList()(i, 0) << " " << _graph->GetEdgeList()(i,
    //     1) << " " << _er(i) << std::endl;
    // }
    ofs << _er;
  }
  ///
  /// Sampling a sparsifier from a dense graph based on calculate effective
  /// weight. As this is a random algorithm, it may need several attempts to get
  /// an acceptable graph.
  ///
  virtual inline gSparse::Graph GetSparsifiedGraph() {
    if (_computeInfo == gSparse::NOT_COMPUTED) {
      throw std::logic_error("SpectralSparsifier by ER: User must run Compute "
                             "before GetSparsifiedGraph()");
    }
    ...
    for (std::size_t i = 0; i != _er.rows(); ++i) {
      double temp = _er(i, 0) * _graph->GetWeightList()(i) * _c *
                    std::log(_graph->GetNodeCount()) / std::pow(_eps, 2);
      double prob = 1.0f < temp ? 1.0f : temp;
      samplingWeights.push_back(prob);
    }

    // Build a discrete distribution based on sampling weight
    std::discrete_distribution<> samplingDistribution(samplingWeights.begin(),
                                                      samplingWeights.end());

    // The algorithm samples O(n log n / ep^2) times edges
    std::size_t samplingCount = static_cast<std::size_t>(
        std::ceil(_graph->GetNodeCount() * std::log(_graph->GetNodeCount()) /
                  std::pow(_eps, 2)));
    ...
    for (std::size_t i = 0; i != samplingCount; ++i) {
      ...
      sparsifiedMap[edgeIndex] +=
          _graph->GetWeightList()(edgeIndex) / samplingWeights[edgeIndex];
    }
  ...
  /// \param C        C hyper-parameter of Spectral Sparsifier by Effective
  /// Resistance.
  ///                 As described by paper. This should be a large constant.
  ...
  inline gSparse::SpectralSparsifier::ER_METHODS GetERPolicy() const {
    return _erPolicy;
  }
