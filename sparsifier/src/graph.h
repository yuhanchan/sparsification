...
public:
  float *get_out_weights() const {
    if (!std::is_same<NodeID_, DestID_>::value) { // weighted graph
      float *out_weights_ = new float[num_nodes_];
      for (int i = 0; i < num_nodes_; i++)
        out_weights_[i] = 0;
      for (int i = 0; i < num_nodes_; i++) {
        for (DestID_ *j = out_index_[i]; j < out_index_[i + 1]; j++) {
          out_weights_[*j] += j->w;
        }
      }
      return out_weights_;
  ...
  CSRGraph(int64_t num_nodes, DestID_ **index, DestID_ *neighs)
    ...
    // if (!std::is_same<NodeID_, DestID_>::value) { // weighted graph
    //   out_weights_ = new int64_t [num_nodes_];
...
#pragma omp parallel for
  ...
  void PrintIthNeighbor(NodeID_ v, int i) {
    int num_neighbors_v = this->out_index_[v + 1] - this->out_index_[v];
    assert(num_neighbors_v > i);
    std::cout << "Neighbor number " << i << " is: " << *(out_index_[v] + i)
              << std::endl;
  }

  void PrintIthElementofEL(int i) const {
    std::cout << i << "th element of EL: " << out_neighbors_[i] << std::endl;
  }

  int64_t find_idx_of_dest(NodeID_ v, NodeID_ v2) {
    int64_t i;
    int64_t lookup_size = this->out_index_[v + 1] - this->out_index_[v];
    for (i = 0; i < lookup_size; i++) {
      if (*(out_index_[v] + i) == v2)
        return i;
    }
    return -1;
  }

  void SetIthIndex(int64_t i) { out_neighbors_[i] = -1; }

  void SetIthNeighborID(NodeID_ v, uint32_t id) {
    // std::cout << "----------\n";
    // std::cout << "nodeid: " << v << ", id: " << id << std::endl;
    // std::cout << "*out_index: " << *out_index_[v] << std::endl;
    // std::cout << "*out_index + idx: " << *(out_index_[v] + id) << std::endl;
    // std::cout << "----------\n";
    *(out_index_[v] + id) = -1;
  }
  ...
  void PrintEdges() const {
    for (uint32_t i = 0; i < num_edges_; ++i)
      std::cout << "edge: " << out_neighbors_[i] << std::endl;
  }

  int64_t *GetPrunedEdgeIDs(int64_t num_pruned_edges) const {
    int64_t *pruned_edge_indices = new int64_t[num_pruned_edges];
    int64_t idx = 0;
    for (int64_t i = 0; i < num_edges_; ++i) {
      if (out_neighbors_[i] < 0) {
        pruned_edge_indices[idx] = i;
        idx++;
      }
    }
