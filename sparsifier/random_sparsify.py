...
def random_sparsify(dataset, dataset_name, drop_rate):
    ...
    """
    myLogger.info(
        f"Getting random prune file with prune rate {drop_rate} for {dataset_name}"
    )
    prune_file_path = osp.join(
        osp.dirname(osp.abspath(__file__)),
        f"../data/{dataset_name}/pruned/random/{drop_rate}/edge_selection.npy",
    )
    duw_el_path = osp.join(
        osp.dirname(osp.abspath(__file__)),
        f"../data/{dataset_name}/pruned/random/{drop_rate}/duw.el",
    )
    uduw_el_path = osp.join(
        osp.dirname(osp.abspath(__file__)),
        f"../data/{dataset_name}/pruned/random/{drop_rate}/uduw.el",
    )
    if osp.exists(prune_file_path):
        ...
        myLogger.info(
            message=f"Prune file not exist, generating random prune file with drop rate {drop_rate}"
        )
        ...
        edge_selection = torch.tensor(
            np.random.binomial(1, 1.0 - drop_rate, size=size)
        ).type(torch.bool)
    ...
    if not osp.exists(duw_el_path):
        np.savetxt(
            duw_el_path, data.edge_index.numpy().transpose().astype(int), fmt="%i"
        )
...
def cpp_random_sparsify(dataset_name, drop_rate, config=None, directed=True):
    ...
    myLogger.info(
        f"Generatin random prune file with drop_rate {drop_rate} for {dataset_name}"
    )
    ...
    if directed:
        input_el_path = os.path.join(
            current_file_dir, f"../data/{dataset_name}/raw/duw.el"
        )
        os.system(
            f"./bin/prune -f {input_el_path} -q random -p {drop_rate} -o {duw_el_path}"
        )
    else:
        input_el_path = os.path.join(
            current_file_dir, f"../data/{dataset_name}/raw/uduw.el"
        )
        os.system(
            f"./bin/prune -f {input_el_path} -q random -p {drop_rate} -o {uduw_el_path}"
        )
    os.chdir(cwd)
...
def cpp_random_sparsify_all(dataset_name, config, directed=True):
    with ProcessPoolExecutor(max_workers=2) as executor:
        ...
        for drop_rate in config[dataset_name]["drop_rate"]:
            futures[
                executor.submit(
                    el_random_sparsify, dataset_name, drop_rate, config, directed
                )
            ] = drop_rate
