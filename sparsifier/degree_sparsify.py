...
def degree_sparsify(
    dataset, dataset_name, in_or_out, degree_thres, config=None, recompile=False
):
    """
    Input:
        dataset: PygDataset
    ...
    myLogger.info(
        f"Getting {in_or_out}_degree prune file with threshold {degree_thres} for {dataset_name}"
    )
    if (
        config == None
        or str(degree_thres)
        ...
        myLogger.info(
            message=f"No config found for {dataset_name} with threshold {degree_thres}, edge_selection will be generated, but will not be saved to file"
        )
        prune_file_path = None
        duw_el_path = None
        uduw_el_path = None
    else:
        prune_file_path = os.path.join(
            current_file_dir,
    ...
    if prune_file_path is not None and osp.exists(prune_file_path):
        ...
        myLogger.info(
            message=f"Prune file not exist, generating {in_or_out}_degree prune file with threshold {degree_thres}"
        )
        if recompile:
        ...
        os.chdir(current_file_dir)
        os.system(
            f"./bin/prune -f {input_duw_el_path} -q {in_or_out}_threshold -x {degree_thres} -o {tmpfile.name}"
        )
        os.chdir(cwd)
        if dataset_name in ["Reddit", "Reddit2", "ogbn_products"]:
            edge_selection = torch.ones(dataset.data.edge_index.shape[1]).type(
                torch.bool
            )
