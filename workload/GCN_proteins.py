...
def GCN_proteins(train_data, test_data, split_idx, evaluator, dataset_name, **kwargs):
    ## Default parameters
    # model parameters
    lr = 0.01
    dropout = 0.0
    ...
    use_sage = False
    # run parameters
    experiment_dir = PROJECT_HOME + "/experiments/GCN/" + dataset_name
    ...
    use_sage = kwargs.get("use_sage", use_sage)
    if use_sage:
        experiment_dir = PROJECT_HOME + "/experiments/SAGE/" + dataset_name
    ...
    assert isinstance(use_sage, bool)
    ...
    if use_sage:
        model = SAGE_NET(
            train_data.num_features, hidden_channels, 112, num_layers, dropout
        ).to(device)
    else:
        model = GCN_NET(
            train_data.num_features, hidden_channels, 112, num_layers, dropout
    ...
    test_scalars = []
    test_columns = ["Run", "Epoch", "Loss", "Train ROCAUC", "Valid ROCAUC", "Test ROCAUC"]
    if experiment_dir is not None:
    ...
    for run in range(runs):
        ...
        for epoch in range(1, 1 + epochs):
            ...
            if epoch % eval_each == 0:
                ...
                test_scalars.append([run, epoch, loss, train_rocauc, valid_rocauc, test_rocauc])
                if experiment_dir is not None:
