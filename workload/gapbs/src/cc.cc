...
#pragma omp parallel for schedule(dynamic, 16384)
  ...
  Compress(g, comp);
  cout << "CPU Time: " << (double)(clock() - start) / CLOCKS_PER_SEC
       << " seconds" << endl;
  return comp;
}
...
void PrintAllCompStats(const Graph &g, const pvector<NodeID> &comp) {
  cout << endl;
  unordered_map<NodeID, NodeID> count;
  for (NodeID comp_i : comp)
    count[comp_i] += 1;
  ...
  sort(count_vector.begin(), count_vector.end(),
       [](const pair<NodeID, NodeID> &a, const pair<NodeID, NodeID> &b) {
         return a.second > b.second;
       });

  cout << "All clusters" << endl;
  for (auto kvp : count_vector)
    cout << kvp.first << ":" << kvp.second << endl;
  cout << "There are " << count.size() << " components" << endl;
}

void PrintComp0(const Graph &g, const pvector<NodeID> &comp) {
  cout << endl;
  unordered_map<NodeID, NodeID> count;
  for (NodeID comp_i : comp)
    count[comp_i] += 1;
  ...
  // print out edges with nodes from the largest component
  for (NodeID n = 0; n < g.num_nodes(); n++) {
    if (comp[n] == count_vector[0].first) {
      for (NodeID neigh : g.out_neigh(n)) {
        cerr << n << " " << neigh << endl;
      }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if (cli.get_component_0()) {
    BenchmarkKernel(cli, g, CCBound, PrintComp0, CCVerifier);
  } else {
    BenchmarkKernel(cli, g, CCBound, PrintAllCompStats, CCVerifier);
  }
  return 0;
}
