...
SparseMatrixCSC lap(SparseMatrixCSC& a){
    ...
    for (int i = 0; i < res.m; i++) {
        ...
        for (int j = res.col_ptr[i]; j < res.col_ptr[i+1]; j++) {
            ...
            if (res.row_ind[j] == i){
                cout << "Diagonal element must be zero " << i << " " << j << " " << res.row_ind[j] << " " << res.val[j] << endl;
                exit(1);
            }
            s += res.val[j];
            res.val[j] = -res.val[j];
        ...
        // cout << "i: " << i << endl;
        // assert(res.nnz == a.nnz+i+1);
        // assert(res.row_ind.size() == a.row_ind.size()+i+1);
        // assert(res.val.size() == a.val.size()+i+1);
        // assert(res.col_ptr[res.col_ptr.size()-1] == a.col_ptr[res.col_ptr.size()-1]+i+1);
    }
    return res;
}
...
vector<vector<double>> approxchol_lapGreedy(SparseMatrixCSC& a, vector<vector<double>>& bs){
    // cout << "row_ind[80255]: " << a.row_ind[80255] << endl;
    cout << "HERE" << endl;
