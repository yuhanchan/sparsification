...
mutable struct LDLinv{Tind,Tval}
    col::Array{Tind,1} # the ith element shows which column is eliminated during the ith iteration
    colptr::Array{Tind,1} # a range of rows be eliminated during the elimination of the ith iteration
    rowval::Array{Tind,1} # the content of all ranges
    fval::Array{Tval,1} # the factor to do the elimination
    d::Array{Tval,1} # the degree to apply for Dinv
end
...
mutable struct ApproxCholPQ{Tind}
    ...
    lists::Array{Tind,1}
    minlist::Int
    nitems::Int # the # of elements still in the priority queue
    n::Int

    deg1::Int #number of elements that have degree==1
end
